...
def make_samples(model, avg, scale, norm_scale, zdim, device, imageSize, image_array, pca, n_events=500, real=True, fake = True):
    ...
    gen_sample = model.decode(sample).cpu()
    #print gen_sample.shape
    #sample_new = gen_sample.view(n_events, 1, imageSize, imageSize).detach().numpy().squeeze()
    sample_new = gen_sample.detach().numpy().squeeze()
    fake_images_array = pca.inverse_transform(unnormalize(sample_new, scale = scale, norm_scale = norm_scale)).reshape(n_events,1,imageSize,imageSize).squeeze()
    
    
    randints = np.random.randint(low = 0,high = image_array.shape[0], size = n_events)
    real_images_array = unnormalize(image_array[randints].squeeze(),norm_scale = norm_scale, scale = scale)
    print real_images_array.shape
...
def samples(source, epoch, zdim,  beta, norm_scale , scale, imageSize, last_decode_act, device, pca, save_dir=None, rows=2, columns=5 ):
    xran = (-50,50)
    yran = (-50,50)
    extent = xran + yran
    ...
    if type(source).__module__ == 'numpy':
        ...
        #sample_conds = torch.cat((sample,energies),1)
        gen_sample = source.decode(sample).cpu()
        #sample = model.decode(sample).cpu()
        #data = unnormalize(gen_sample.view(64, 1, imageSize, imageSize).detach().numpy().squeeze(), norm_scale = norm_scale, scale = scale)
        data = pca.inverse_transform(unnormalize(gen_sample.detach().numpy(), norm_scale  = norm_scale, scale = scale)).reshape(64,1,imageSize,imageSize).squeeze()
